...
function animateClouds() {
    ...
    animationFrameIdClouds = requestAnimationFrame(animateClouds);
}

const cloudImages = [
    'cv/media/static/clouds/cloud1.png',
    'cv/media/static/clouds/cloud4.png',
    'cv/media/static/clouds/cloud7.png',
    'cv/media/static/clouds/cloud10.png',
    'cv/media/static/clouds/cloud22.png'
];
...
window.initClouds = function() {
    ...
    clouds = [];
    if (animationFrameIdClouds) {
        cancelAnimationFrame(animationFrameIdClouds);
    }
...
window.stopClouds = function() {
    if (animationFrameIdClouds) {
        cancelAnimationFrame(animationFrameIdClouds);
        animationFrameIdClouds = null;
    }
    if (cloudContainer) {
        cloudContainer.innerHTML = ''; // Clear clouds from container
    }
};

// Parallax effect on scroll
window.addEventListener('scroll', () => {
    if (clouds.length > 0) { // Only apply parallax if clouds are active
